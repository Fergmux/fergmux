<template>
  <div style="display: flex; align-items: center; padding: 50px">
    ...
    <div style="display: flex; flex-direction: column; margin: 50px">
      <button class="button-light" style="margin-bottom: 20px" @click="main">
        Generate
      </button>
      Grid size: {{ gridSizeRef }}
      <input v-model="gridSizeRef" type="range" min="10" max="50" />
    </div>
  </div>
</template>
  ...
  setup() {
    ...
    function load() {
      ...
      Object.entries(images).forEach(([key, src]) => {
        ...
        image.onload = () => {
          ...
          if (imageCount === Object.entries(images).length) {
            // have all loaded????
    ...
    function main() {
      ...
      function placeTemple(image, size, maxOffset) {
        ...
        ctx.drawImage(
          image,
          templeLocationX * cellWidth,
          templeLocationY * cellHeight,
          cellWidth * size,
          cellHeight * size
        )
      ...
      const smallTempleCount =
        Math.floor(Math.random() * smallTempleRange) + smallTempleRange + 1
      ...
      function makeConnections(cell, image) {
        ...
        const validNeighbours = neighbours.filter(
          (cell) =>
            cell && !visitedCellIds.includes(cell.id) && cellIsNotCrowded(cell)
        )
        const validNeighbourCount = validNeighbours.length
        let pathNeighbours = Array.from(
          { length: validNeighbourCount },
          () => Math.random() >= 0.5
        )
        if (!pathNeighbours.includes(true)) {
          pathNeighbours[
            Math.floor(Math.random() * pathNeighbours.length)
          ] = true
        }
