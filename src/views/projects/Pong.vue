...
let mousePos: { x?: number; y?: number } = {} // make mouse position global
...
function reset(): [Ball, number[]] {
  ...
  var xspeed = (Math.sign(Math.random() - 0.5) * maxspeed * canvas.width) / 800
  ...
  drawPaddle(false, mousePos.y as number) // draw the player paddle at the mouse location
  return [ball, pady]
}

onMounted(() => {
  canvas = document.getElementById('mycanvas') as HTMLCanvasElement
  gl = initWebGL() as CanvasRenderingContext2D
...
function getPaddlePos(ball: Ball, pady: number[]) {
...
function checkPaddleHit(ball: Ball, pady: number[]): [Ball, number[]] {
  let padd
  let edge
  let pleft
  let pright
  ...
  if (ball.vx < 0) {
    ...
    padd = limitPos(padd as number) // limit the paddle position
...
function drawPaddle(right: boolean, y: number) {
...
// main animation loop
function animate(ball: Ball, pady: number[]) {
